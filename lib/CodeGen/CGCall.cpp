...
CodeGenTypes::arrangeCXXStructorDeclaration(const CXXMethodDecl *MD,
  ...
  SmallVector<CanQualType, 16> argTypes;
  bool isCLIRecord = MD->getParent()->isCLIRecord();
  ...
  auto CGInfo = arrangeLLVMFunctionInfo(resultType, /*instanceMethod=*/true,
                                        /*chainCall=*/false, argTypes, extInfo,
                                        required);

  if (isCLIRecord && isConstructor) {
    auto CC = llvm::CallingConv::CIL_NewObj;
    if (MD->getParent()->getCLIData()->Type == CLI_RT_ValueType)
